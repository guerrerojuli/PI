...
/* En este test asumimos que en caso de estar repetidos deja la primer aparición
 *del elemento, no la última
...
int eliminaRepetidos(const int original[], int dimOrig, int resultado[]) {
  ...
  for (int i = 0; i < dimOrig; i++) {
    int found = 0;
    // recorro el array de resultados en busca de repetidos
    for (int j = 0; j < dim; j++) {
      if (original[i] == resultado[j])
        found = 1; // si hay un repetido prendo la flag
    }
    if (!found)
      resultado[dim++] = original[i];
  }
  return dim;
}
