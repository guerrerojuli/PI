...
int calcularCasilla(unsigned int dim, const int m1[][dim], const int m2[][dim],
                    int fila, int columna) {
  int sumatoria = 0;
  for (int i = 0; i < dim; i++)
    sumatoria += m1[fila][i] * m2[i][columna];
  return sumatoria;
}

void productoMat(unsigned int dim, const int m1[][dim], const int m2[][dim],
                 int m3[][dim]) {
  for (int i = 0; i < dim; i++)
    for (int j = 0; j < dim; j++)
      m3[i][j] = calcularCasilla(dim, m1, m2, i, j);
}
