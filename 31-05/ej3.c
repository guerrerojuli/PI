...
queueADT newQueue(void) { return calloc(1, sizeof(struct queueCDT)); }
...
int isEmpty(queueADT q) { return (q->first == NULL); }
...
void toBegin(queueADT q) { q->current = q->first; }

int hasNext(queueADT q) { return (q->current != NULL); }
